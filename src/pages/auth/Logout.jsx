...
const Logout = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const userType = searchParams.get('USER_TYPE')?.toUpperCase() ?? 'CLIENT';
      const target = userType === 'ADMIN' ? '/auth/login/admin' : '/auth/login/client';
      navigate(target);
    }, 2000);
          ...
          >
            ...
            <motion.div variants={itemVariants} className="tw:text-center">
              <div className="tw:flex tw:items-center tw:justify-center tw:gap-2 tw:mb-2">
                <LogOut className="tw:w-5 tw:h-5 tw:text-gray-400" />
                <span className="tw:text-lg tw:font-semibold tw:text-slate-200">Logging out...</span>
              </div>
